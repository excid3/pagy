...
```rb
# this will load the "de" built-in locale:
Pagy::Frontend::I18N.load(locale: 'de') 
```

```rb
# this will load the "de" locale defined in the custom file at :filepath:
Pagy::Frontend::I18N.load(locale: 'de', 
                          filepath: Rails.root.join('locales', 'pagy-custom-de.yml') 
```

```rb
# this will load the "de", "en" and "es" built-in locales:
Pagy::Frontend::I18N.load({locale: 'de'}, {locale: 'en'}, {locale: 'es'})
 
# the first argument in the argument list will be used also as the `:default_locale`
Pagy::Frontend::I18N[:default_locale]
#=> 'de'

# you can also set it explicitly to a different already-loaded locale:
Pagy::Frontend::I18N[:default_locale] = 'es'
```

```rb
# this will load the "en" built-in locale, a custom "es" locale, and a totally custom locale complete with the :pluralize proc:
Pagy::Frontend::I18N.load({locale: 'en'}, 
                          {locale: 'es', 
                           filepath: 'path/to/custom-es.yml'},
                          {locale: 'xyz',  # not built-in
                           filepath: 'path/to/xyz.yml',
                           pluralize: lambda{|count| ... } )
```

**Notice**: You should use a custom `:pluralize` proc only for pluralization types not included in the built-in [plurals.rb](https://github.com/ddnexus/pagy/blob/master/lib/locales/plurals.rb)
...
In case the `@pagy_locale` is not set to any loaded locale, then the `:default_locale` will be used for the translation.
